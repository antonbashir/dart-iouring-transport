...
void transport_test() {
  ...
  rlist_add_entry(list, &item2, link);
  rlist_add_entry(list, &item3, link);

  struct rlist* iterator = rlist_next(list);
  struct item* iterator_item = rlist_entry(iterator, struct item, link);
  printf("value: %d\n", iterator_item->value);

  iterator = rlist_next(iterator);
  iterator_item = rlist_entry(iterator, struct item, link);
  printf("value: %d\n", iterator_item->value);
