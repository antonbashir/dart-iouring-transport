...
void testTcp({
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clientsPool]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      worker.servers.tcp("0.0.0.0", 12345, (channel) => channel.read(), (stream) => stream.listen(onError: (error) => print(error), (event) => event.respond(serverData)));
...
void testUdp({
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clients]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      final worker = TransportWorker(input);
      await worker.initialize();
      worker.servers.udp("0.0.0.0", 12345, (channel) => channel.receiveMessage(), (stream) => stream.listen(onError: (error) => print(error), (event) => event.respond(serverData)));
