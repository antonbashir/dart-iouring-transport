...
void main() {
  testTcp(
    ...
    clientTimeout: Duration(seconds: 5),
    workerFlags: ringSetupSqpoll,
  );
  // group("[tcp]", () {
...
void testTcp({
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clientsPool]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      worker.servers.tcp(
        configuration: TransportDefaults.tcpServer().copyWith(readTimeout: serverTimeout),
        "0.0.0.0",
        12345,
        (channel) => channel.read(),
        (stream) => stream.listen(onError: (error) => print(error), (event) => Future.delayed(Duration(seconds: 5)).then((value) => event.respond(serverData))),
      );
      final clients = await worker.clients.tcp(
        "127.0.0.1",
        12345,
        configuration: TransportDefaults.tcpClient().copyWith(
          pool: clientsPool,
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clients]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      worker.servers.udp(
        ...
        (stream) => stream.listen(onError: (error) => print(error), (event) {
...
void testUnixStream({
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clientsPool]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      worker.servers.unixStream(serverSocket.path, (channel) => channel.read(), (stream) => stream.listen(onError: (error) => print(error), (event) => event.respond(serverData)));
...
void testUnixDgram({
  ...
  test("[index = $index, listeners = $listeners, workers = $workers, clients = $clients]", () async {
    ...
    await transport.run(transmitter: done.sendPort, (input) async {
      ...
      worker.servers.unixDatagram(serverSocket.path, (channel) => channel.receiveMessage(), (stream) => stream.listen(onError: (error) => print(error), (event) => event.respond(serverData)));
      final responseFutures = <Future<List<int>>>[];
      for (var clientIndex = 0; clientIndex < clients; clientIndex++) {
