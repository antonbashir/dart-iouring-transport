...
class TransportErrorHandler {
  ...
  void _handleRead(int bufferId, int fd, int event, int result) {
    ...
    if (result == -ECANCELED) {
      _callbacks.notifyInboundError(bufferId, TransportCanceledException(event: TransportEvent.ofEvent(event)));
      return;
    }
  ...
  void _handleClientReadReceiveCallbacks(int bufferId, int fd, int event, int result) {
    ...
    if (result == -ECANCELED) {
      _callbacks.notifyOutboundError(bufferId, TransportCanceledException(event: TransportEvent.ofEvent(event)));
      return;
    }
